...
?>
<?php
if(isset($_POST['lek_zmien_dane_pacjenta'])){
    if(isset($_GET['id_pacjenta'])){
        $zapytanie_pacjent = $link->prepare("SELECT id_pacjenta FROM pacjent WHERE id_pacjenta=? AND id_lekarza=?");
        ...
        if(!$zapytanie_pacjent){
            ...
            header("Location: lek-wyswietl-dane-pacjenta.php?id_pacjenta=".$_GET['id_pacjenta']."");
            exit();
        }
        ...
        if($aktualizuj_dane_pacjenta){
            ...
            $_SESSION['blad_zmiana_danych_pacjenta'] = 1;
            header('Location: lek-wyswietl-dane-pacjenta.php?id_pacjenta='.$id_pacjenta.'');
            exit();
        }
    }
    else{
