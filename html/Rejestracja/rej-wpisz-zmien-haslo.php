...
?>
<?php
if(isset($_POST['rej_zmien_haslo'])){
    ...
    if(!$zapytanie_haslo){
        $_SESSION['blad_zmien_haslo'] = 1;
        header('Location: rej-zmien-haslo.php');
        exit();
    }
    ...
    if($porownaj_haslo){
        ...
        if($nowe_haslo === $nowe_haslo_potwierdz){
            ...
            if(!$zmien_haslo){
                ...
                $aktualizuj_date_zmiany_hasla = $link->prepare("UPDATE rejestrator SET data_ostatniej_zmiany_hasla=? WHERE id_rejestratora=?");
